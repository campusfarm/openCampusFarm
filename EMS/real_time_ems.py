...
from connections.charger.enphase_automation import (
    charger_off,
    charger_on,
    plugged_in_and_charging,
)
from connections.charger.solArk_inverter import get_inverter_data
from connections.ev_battery import check_battery
from coolbot import change_setpoint, get_coolbot_temp, get_sensor_temp
...
def functional_test_save():
    ...
    with open("output_1210.txt", "a") as file:
        ...
        file.write(
            f"ev_charge: {ev_charge}, ev_miles_left: {ev_miles_left}, pv_output: {pv_output}, cooler_indoor_temp: {cooler_indoor_temp}, ev_connected: {ev_connected}\n"
        )
        file.write(
            f"total_power: {total_power}, power_map: {power_map}, cooler_dirty_periods: {cooler_dirty_periods}\n"
        )
        file.write(f"ev_charging: {ev_charging}, driving: {driving}\n")
        file.write(f"ev_p5: {ev_p5}, cooler_load: {cooler_load}\n")
        file.write(
            f"ev_miles_travelled: {ev_miles_travelled}, grid_power: {grid_power}, solar_power_used: {solar_power_used}\n\n"
        )
...
def toggle_vent():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Failed to toggle vent. Status code: {response.status_code}, Response: {response.text}"
            )
...
def change_vent():
    ...
    if (
        outdoor_temp < cooler_indoor_temp
        and cooler_indoor_temp > TMIN
        and not vent_open
    ):
        toggle_vent()
    else:
...
def star_adjust_temp_setpoint_coolth():
    ...
    with open("output_1210.txt", "a") as file:
        ...
        if CURRENT_SETPOINT != SETPOINT_COOLTH:
            ...
            file.write(
                f"{realtime}: send_cooler_decision({SETPOINT_COOLTH}, CURRENT_SETPOINT != SETPOINT_COOLTH\n"
            )
            ...
            if cooler_indoor_temp <= SETPOINT_COOLTH + 2:
                ...
                if coolth_timer == 0:
                    ...
                    file.write(
                        f"{realtime}: send_cooler_decision({SETPOINT_DEFAULT}, max coolth time limit hit!\n"
                    )
                    functional_test_save()
                ...
                file.write(f"Decision:\n")
                file.write(
                    f"{realtime}: else coolth timer not >= MAX COOLTH TIME LIMIT, do nothing!\n"
                )
                functional_test_save()
...
def ems():
    ...
    global \
        total_emissions_ems, \
        total_emissions_no_ems, \
        total_emissions_baseline, \
        EREMS, \
        grid_load_no_ems, \
        total_load_baseline, \
        ev_load_E_no_ems, \
        cooler_load_E_no_ems, \
        additional_load_E
    global \
        emissions_saved_from_ev, \
        emissions_saved_from_ems, \
        emissions_saved_from_solar, \
        ev_grid_emissions_no_ems, \
    ...
    with open("output_1210.txt", "a") as file:
        if RULE_BASED_MODE == True and OPTIMIZATION_MODE == False:
            ...
            if is_realtime_in_clean_periods(realtime, ev_clean_periods):
                print(
                    f"Current time {realtime.strftime('%H:%M')} is within a clean period."
                )
                if ev_connected:
                    if ev_charging == False:
                        ...
                        file.write(f"Decision:\n")
                        file.write(
                            f"{realtime}: send_charging_decision(True), already charging, do nothing\n"
                        )
                        functional_test_save()

            else:
                print(
                    f"Current time {realtime.strftime('%H:%M')} is NOT within a clean period."
                )
                if ev_connected:
                    if driving == True:
                        ...
                        file.write(
                            f"{realtime}: back from drive, generate new clean charging schedule\n"
                        )
                    ...
                    if ev_charging:
                        ...
                        file.write(f"Decision:\n")
                        file.write(
                            f"{realtime}: send_charging_decision(False) - not in clean period turning charging off)\n"
                        )
                        functional_test_save()
                    else:
                        file.write(f"Decision:\n")
                        file.write(
                            f"{realtime}:not in clean period, not charging, do nothing\n"
                        )
                        functional_test_save()
                else:
                    driving = True
            ...
            if grid_power == 0 and (pv_output > total_power):  # daytime
                if cooler_indoor_temp < TMIN:
                    if CURRENT_SETPOINT != SETPOINT_ECON:
                        ...
                        file.write(f"Decision:\n")
                        file.write(
                            f"{realtime}: setting cooler decision to ECON - too cold inside the cooler!\n"
                        )
                        functional_test_save()
                else:
                ...
                if cooler_indoor_temp < TMIN:
                    ...
                    is_realtime_in_clean_periods(realtime, ev_clean_periods)
                    and CURRENT_SETPOINT != SETPOINT_DEFAULT
                ):
                    # TODO do some coolth?
                    send_cooler_decision(SETPOINT_DEFAULT)
                    file.write(f"Decision:\n")
                    file.write(
                        f"{realtime}: send_cooler_decision({SETPOINT_DEFAULT}, not in a cooler dirty period\n"
                    )
                    functional_test_save()
                    ...
                    if CURRENT_SETPOINT != SETPOINT_ECON:
                        ...
                        file.write(f"Decision:\n")
                        file.write(
                            f"{realtime}: send_cooler_decision({SETPOINT_ECON}, in a cooler dirty period\n"
                        )
                        functional_test_save()
                        CURRENT_SETPOINT = SETPOINT_ECON
                        ...
                        if cooler_indoor_temp >= SETPOINT_ECON - 2:
                            ...
                            if econ_timer == 0:
                                ...
                                file.write(f"Decision/EVENT:\n")
                                file.write(
                                    f"{realtime}: stopping econ timer reached limit \n"
                                )
                                file.write(
                                    f"{realtime}: send_cooler_decision({SETPOINT_DEFAULT}\n"
                                )
                                functional_test_save()
                                CURRENT_SETPOINT = SETPOINT_DEFAULT
                                econ_timer = 0
                                ...
                                file.write(
                                    f"{realtime}: econ_timer < max econ time limit, do nothing!\n"
                                )
                                functional_test_save()

                        else:  ############### DELETE THIS BLOCK WHEN FOR FINAL CODE ONLY FOR DEBUGGING ###################
                            file.write(
                                f"{realtime}: else cooler_indoor temp not >= setpoint econ -2, do nothing!\n"
                            )
                            functional_test_save()
            ...
            file.write(
                f"{realtime}: send_charging_decision({optimized_decision_charging_decision}))\n"
            )
        ...
        aoer = aoer_MWh / 1000  ###### converting unit to be over kWh
        ...
        total_emissions_no_ems = (grid_load_no_ems * aoer) * MT_co2_to_lbs_co2
        total_emissions_ems = (total_emissions_no_ems - EREMS) * MT_co2_to_lbs_co2
        total_emissions_baseline = (total_load_baseline * aoer * MT_co2_to_lbs_co2) + (
            combustion_vehicle_mies * 1.30
        )
        ...
        if ev_load_E_no_ems == 0:
            ...
            ev_grid_emissions_no_ems = (
                ev_load_E_no_ems / total_load_no_ems
            ) * total_emissions_no_ems

        emissions_saved_from_ev = (
            (combustion_vehicle_mies * 1.30) - ev_grid_emissions_no_ems
        ) * MT_co2_to_lbs_co2
        emissions_saved_from_solar = (pv_e * aoer) * MT_co2_to_lbs_co2
        ...
        try:
            ...
            if wifi_status:
                ...
                if OPTIMIZATION_MODE == True and RULE_BASED_MODE == False:
                    ...
                    time.sleep(
                        300
                    )  # modify this : if time interval 5min? should be time_interval * 60s

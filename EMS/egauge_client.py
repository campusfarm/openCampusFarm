...
class EGaugeClient:
    ...
    def get_cooler_power(self):
        data = self.get_live_data()
        return data["energy"]["S8*L1"]["rate"] + data["energy"]["-S8*L2"]["rate"] # config is S8*L1 and -S8*L2, negative means consume
    
    def get_evcharger_power(self):
        data = self.get_live_data()
        return data["energy"]["S5*L2"]["rate"] + data["energy"]["-S5*L1"]["rate"] #config is S5*L2 and -S5*L1, negative means consume?
